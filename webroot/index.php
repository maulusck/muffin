...
function route_v3(array $segs, string $method): void
{
    ...
    if ($s1 === 'registration') {
        ...
        if ($s3 === 'index.json' || $s3 === '') {
            v3_registration_index($id);

            return;
        }
        if (str_ends_with($s3, '.json')) {
            v3_registration_leaf($id, basename($s3, '.json'));
...
function v3_service_index(): void
{
    ...
    json_out([
        ...
        'resources' => [
            ...
            [
                '@id' => "$b/v3/registration/",
                '@type' => 'RegistrationsBaseUrl',
                'comment' => 'Registration base',
            ],
            [
                '@id' => "$b/v3/registration/",
            ...
            [
                '@id' => "$b/v3/flatcontainer/",
                '@type' => 'PackageBaseAddress/3.0.0',
                'comment' => 'Flat container',
            ],
...
function v3_search(): void
{
    ...
    if ($q !== '') {
        $all = array_values(
            array_filter(
                $all,
                fn ($p) => str_contains(strtolower($p['id']), $q) ||
...
function v3_registration_leaf(string $lid, string $version): void
{
    ...
    $pkgs = array_filter(
        $all,
        fn ($p) => strtolower($p['id']) === $lid && $p['version'] === $version,
    );
    $pkg = reset($pkgs);
    if (! $pkg) {
        json_404();
...
function serve_single_entity(string $id, string $version): void
{
    ...
    $pkgs = array_filter(
        $all,
        fn ($p) => strcasecmp($p['id'], $id) === 0 && $p['version'] === $version,
    );
    $pkg = reset($pkgs);
    if (! $pkg) {
        http_response_code(404);
...
function serve_get_updates(): void
{
    ...
    foreach ($ids as $i => $clientId) {
        ...
        $candidates = array_filter(
            $all,
            fn ($p) => strcasecmp($p['id'], $clientId) === 0 &&
...
function serve_download(string $id, string $version): void
{
    ...
    if ($version !== '') {
        ...
        if (! $file) {
            $c = "$dir/".strtolower($id).".$version.nupkg";
            if (file_exists($c)) {
                $file = $c;
            }
        }
...
function parse_dependencies($deps): string
{
    ...
    foreach ($deps->group ?? [] as $group) {
        ...
        foreach ($group->dependency ?? [] as $dep) {
            $out[] =
                (string) ($dep['id'] ?? '').':'.(string) ($dep['version'] ?? '').':'.$tf;
        }
    }
    ...
    if (isset($_GET['searchTerm'])) {
        ...
        if ($term !== '') {
            $packages = array_values(
                array_filter(
                    $packages,
                    fn ($p) => str_contains(strtolower($p['id']), $term) ||
...
function apply_filter(array $packages, string $filter): array
{
    ...
    if (preg_match("/startswith\s*\(\s*Id\s*,\s*'([^']+)'\s*\)/i", $filter, $m)) {
        ...
        return array_values(
            array_filter($packages, fn ($p) => str_starts_with(strtolower($p['id']), $prefix)),
        );
    }
    ...
    if (preg_match("/IsAbsoluteLatestVersion\s+eq\s+true/i", $filter)) {
        ...
        return array_values(
            array_filter(
                array_map(fn ($p) => compute_flags($p, $all), $packages),
                fn ($p) => $p['isAbsoluteLatestVersion'],
            ),
        );
    }
...
function feed_open(string $title, int $total): void
{
    ...
    $updated = date('c');
    $inlinecount =
        isset($_GET['$inlinecount']) && $_GET['$inlinecount'] === 'allpages'
